...
import { Brain, HelpCircle, AlertCircle, CheckCircle2, XCircle, ArrowRight } from 'lucide-react';
...
export default function LogicPuzzles({ user, onUpdateUser }: LogicPuzzlesProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [selectedIdx, setSelectedIdx] = useState<number | null>(null);
  const [isAnswered, setIsAnswered] = useState(false);
  const [isCorrect, setIsCorrect] = useState(false);

  const current = MANTIQIY_MASALALAR[currentIndex];
  ...
  const handleNext = () => {
    if (currentIndex < MANTIQIY_MASALALAR.length - 1) {
    ...
    <div className="max-w-3xl mx-auto space-y-8 pb-12">
      ...
      <div className="grid grid-cols-2 gap-4">
         <div className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-3">
            ...
            <div className="text-sm font-medium text-slate-500">Savol {currentIndex + 1} / {MANTIQIY_MASALALAR.length}</div>
         </div>
         <div className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-3">
